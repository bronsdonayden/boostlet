    ...
    static filter(pixels, width, height, kernel) {
        ...
        for(let y = 0; y < height; y++)for(let x = 0; x < width; x++){
            ...
            for(let ky = 0; ky < kernelSize; ky++)for(let kx = 0; kx < kernelSize; kx++){
                const srcX = Math.min(Math.max(x + kx - halfKernelSize, 0), width - 1);
                const srcY = Math.min(Math.max(y + ky - halfKernelSize, 0), height - 1);
...
class $c41144187e031f80$export$a1c8e230980baa0 {
   ...
   */ init(name, instance) {
        ...
        if (this.framework) console.log("Found", this.framework.name, "!");
